...
class ShoppingViewModel(
    ...
    init {
        viewModelScope.launch {
            repo.shoppingItems.collectLatest { newEntries ->
                entries.clear()
    ...
    fun executeOfflineAction(
        ...
        viewModelScope.launch {
            entries.forEach {
                when (action) {
                    ShoppingListEntryOfflineActions.CHECK -> repo.toggleCheck(client, it.id, true)
                    ShoppingListEntryOfflineActions.UNCHECK -> repo.toggleCheck(client, it.id, false)
                    ShoppingListEntryOfflineActions.DELETE -> repo.delete(client, it.id)
                }
            }
        }
    }
