...
import de.kitshn.db.ShoppingListEntryOfflineActions
import de.kitshn.db.SyncTransactionEntity
import de.kitshn.db.toEntity
...
class ShoppingRepository(
    ...
    val shoppingItems: Flow<List<TandoorShoppingListEntry>> = dao.getAllAsFlow().map { entities ->
        entities.map { it.toTandoorEntry() }
    }
    ...
    suspend fun toggleCheck(client: TandoorClient?, entryId: Int, checked: Boolean) {
        withContext(Dispatchers.IO) {
            ...
            dao.insertTransaction(SyncTransactionEntity(entryId = entryId, action = action))
        }
    ...
    suspend fun delete(client: TandoorClient?, entryId: Int) {
        withContext(Dispatchers.IO) {
            ...
            // log transaction
            dao.insertTransaction(SyncTransactionEntity(entryId = entryId, action = ShoppingListEntryOfflineActions.DELETE))
        }
